...
export function registerMcpTools(server: ToolServer): void {
  ...
  register(server, "search_full_compartments", "Find compartments whose places are all free, over an inclusive date range of at most 31 days. A match is confirmed only when the carriage response lists the requested places inside one compartment; carriages with enough free places but no such confirmation are returned as candidates.", {
    from_station: station, to_station: station, date_from: date, date_to: date, places: z.number().int().min(2).default(4), max_results: z.number().int().min(1).max(500).default(100),
  }, async (args) => usingClient((client) => client.searchFullCompartments(args.from_station, args.to_station, args.date_from, args.date_to, { places: args.places, maxResults: args.max_results })));

...
export interface RzdConfig {
  ...
  baseUrl: string;
  b2bBaseUrl?: string;
  /** Where this server answers, used for the scheme links handed to clients that cannot
...
export const defaultConfig: RzdConfig = {
  ...
  publicBaseUrl: "https://rzd-api.vercel.app",
  timeoutMs: 25_000,
  retryTotal: 3,
  retryBackoffMs: 500,
...
export function makeConfig(config: Partial<RzdConfig> = {}): RzdConfig {
  ...
  for (const [name, url] of [["baseUrl", value.baseUrl], ["b2bBaseUrl", value.b2bBaseUrl], ["schemeImageBaseUrl", value.schemeImageBaseUrl], ["publicBaseUrl", value.publicBaseUrl]] as const) {
...
export function configFromEnvironment(
  ...
  return {
    ...
    ...(env.RZD_PUBLIC_BASE_URL ? { publicBaseUrl: env.RZD_PUBLIC_BASE_URL } : {}),

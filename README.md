...
сайт перекрашивает места под статус. Поэтому номера всегда перекрашиваются в
тёмный, `free_places` заливаются синим, а `selected_places` красным — те же два
цвета, что в легенде ticket.rzd.ru. Без этого картинка нечитаема.

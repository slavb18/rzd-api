...
К первому подтверждённому купе прикладывается чертёж вагона: в ответе приходит
готовая картинка с залитыми синим местами. Отдельно просить «с картинкой» не
нужно — иначе клиент, оставленный без изображения, иллюстрирует ответ
фотографиями чужих вагонов из интернета. Отключается `include_image: false`.
